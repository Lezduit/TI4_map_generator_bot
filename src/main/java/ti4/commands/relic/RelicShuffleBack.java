...
class RelicShuffleBack extends GameStateSubcommand {
    ...
    public void execute(SlashCommandInteractionEvent event) {
        ...
        if (player.hasRelic(relicId)) {
            ...
            boolean success = game.shuffleRelicBack(relicId);
            if (success) {
                String relicName = Mapper.getRelic(relicId).getName();
                MessageHelper.sendMessageToEventChannel(event, "Moved from play area and shuffled " + relicName + "back into deck.");
            } else {
            ...
            if (allRelics.contains(relicId)) {
                boolean succes = game.shuffleRelicBack(relicId);
                if (succes) {
                  String relicName = Mapper.getRelic(relicId).getName();
                  MessageHelper.sendMessageToEventChannel(event, "Unpurged and shuffled " + relicName + " back into the deck.");
                } else {
                  MessageHelper.sendMessageToEventChannel(event, "Could not shuffle relic back into deck.");
                }
            } else {
              MessageHelper.sendMessageToEventChannel(event, "Relic not found in player area or game relics.");
            }            
        }
    }
}
